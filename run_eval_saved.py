...
class EvaluateAll:
    ...
    def __init__(self):
        ...
        self.annotations_path = config['annotations_path2']


    def convert_bb(self, bb):
        return [(bb[0], bb[1]), (bb[0] + bb[2], bb[1] + bb[3])]

    def convert(self, labels, w, h):
        res = []
        for label in labels:
            x = [int(label[0]*w), int(label[1]*h), int(label[2]*w), int(label[3]*h)]
            res.append(x)
        return res
    ...
    def run_evaluation(self):
        ...
        im_list = sorted(glob.glob('detections/yolo5he/*.txt'))
        print(im_list)
        iou_arr = []
        ...
        for im_name in im_list:
            ...
            prediction_list = self.get_annotations(im_name)
            ...
            prediction_list = self.convert(prediction_list, 100, 100)
            annot_list = self.convert(annot_list, 100, 100)
