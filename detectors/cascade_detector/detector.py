...
class Detector:
	...
	cascade1 = cv2.CascadeClassifier(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'cascades', "haarcascade_mcs_leftear.xml"))
	...
	def detect(self, img):
		det_list = self.cascade1.detectMultiScale(img, 1.05, 1)
		if len(det_list) == 0:
			det_list = self.cascade2.detectMultiScale(img, 1.05, 1)
		return det_list
